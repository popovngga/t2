...
@section('content')
    <div class="w-full max-w-md bg-white rounded-xl shadow-md p-6 space-y-6">
        ...
        <form action="{{ route('login.store') }}" method="POST" class="space-y-4" id="register-form">
            @csrf
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700" for="phone">Password</label>
                <input
                    type="password"
                    name="password"
                    id="password"
